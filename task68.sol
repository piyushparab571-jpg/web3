...
contract StorageGasCostVul {
    ...
    //STRUCT ARRAY IN STORAGE
    Data[] public storedData;
    //SIMPLE ARRAY FOR COMPARISION
    uint256[] public storedValues;
    //STORE 1000 VALUES - SIMPLE ARRAY
     function store1000Values() external {
        //Stores 1000 values in a storage.
        for (uint256 i = 0; i < 1000; i++) {
            storedValues.push(i);
        }
     }
     //STORE 1000 VALUES - STRUCT ARRAY
     function store1000Structs() external {
        /* stores 1000 struct objects in a storage array.
        */
        for (uint256 i = 0; i < 1000; i++) {
            storedData.push(
                Data({
     ...
     {
        //Memory array is temporary. It does not permanently modify storage
